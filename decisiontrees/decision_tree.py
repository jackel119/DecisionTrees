...
class DecisionTreeClassifier:
    ...
    def plotTreeUtil(self, isLeft, node, x1, y1, x2, y2):
        ...
        if not node.is_leaf:
            self.plotTreeUtil(True, node.left_node, x1, y1, (x1 + x2) / 2,
                              y2 - 10)
            self.plotTreeUtil(False, node.right_node, (x1 + x2) / 2, y1, x2,
                              y2 - 10)
    ...
    def plotTree(self):
        ...
        self.plotTreeUtil(False, self.root_node.right_node, midx, y1, x2,
                          y2 - 10)
        plt.show()
